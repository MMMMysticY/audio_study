...
class ExtractAudioFeature(nn.Module):
    ...
    def forward(self, filepath):
        waveform, sample_rate = torchaudio.load(filepath) # load file
        ...
        # 返回值是[time, num_mel_bins] -> [1, num_mel_bins, time] 就是[channel, feature_dim, time]
...
def create_transform(audio_config):
    ...
    # transforms的形式是[ExtractAudioFeature -> Delta(optimal) -> CMVN(optimal) -> Postprocess
